...
|:---- |:---- |:---- |:---- |:---- |
|VIRTUAL |  查询时实时计算（<font color=red size=3 face="黑体">只在需要时</font>）	|不占用主表空间	|可索引（索引本身占用空间）	|简单计算、节省主表空间、需要函数索引的场景|
|STORED	 |  插入/更新时计算	|占用主表空间	|可索引	|复杂计算、对计算性能要求高（牺牲存储空间）、需要像普通列一样使用的场景|

{{< /admonition >}}

**VIRTUAL 列的索引**：虽然 VIRTUAL 列的值本身不存储在数据行里，但当你为它创建索引时，MySQL 会物化（存储）索引结构。索引中的数据是预先计算好的，因此查询可以通过扫描这个索引（而不是全表并实时计算）来加速。
...
不同的场景或版本差异，索引的使用效果也不同，所以需要根据实际情况来选择索引，

如果虚拟列没有效果，可以尝试使用STORED列。
